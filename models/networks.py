...
class BMDSSI(Depth):
    def __init__(self, cfg, resize_method=None):
        ...
        if device == 'mps:0':
            state_dict_ssi = torch.load(self.cfg.ssi_weights, map_location=torch.device('mps:0'))
        elif device == 'cpu':
            state_dict_ssi = torch.load(self.cfg.ssi_weights, map_location=torch.device('cpu'))
        else:
